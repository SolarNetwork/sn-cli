...
public class SetControlParameterCmd extends BaseSubCmd<ControlsGroup> implements Callable<Integer> {
	...
	public Integer call() throws Exception {
		try {
			...
			final BasicInstruction instr = new BasicInstruction(null, "SetControlParameter", null, null);
			...
			if (status.getInstructionState() == InstructionState.Completed) {
				System.out.println("Control [%s] set to [%s]".formatted(parentCmd.controlId, value));
				return 0;
			} else if (status.getInstructionState() == InstructionState.Declined) {
				System.out.println(Ansi.AUTO
						.string("@|red Setting [%s] to [%s] was refused.|@".formatted(parentCmd.controlId, value)));
				return 2;
			}
