 ...
 */
@Component
@Command(name = "stream", sortSynopsis = false)
public class StreamFluxCmd extends BaseSubCmd<FluxCmd> implements Callable<Integer> {

	// @formatter:off
	@ArgGroup(exclusive = true, multiplicity = "1")
	TopicOrNodeSource topicOrNodeSource;

	@Option(names = { "-c", "--client-id" },
			description = "a client ID to use")
	String clientIdSuffix = "_" +UUID.randomUUID().toString().replaceAll("-", "").substring(0, 8);

	@Option(names = { "-mode", "--display-mode" }, description = "how to display the CSV data", defaultValue = "PRETTY")
	 ...
	 */
	public StreamFluxCmd(ClientHttpRequestFactory reqFactory, ObjectMapper objectMapper) {
	...
	public Integer call() throws Exception {
		...
		try (final var executor = Executors.newCachedThreadPool();
			...
			mqttConfig.setClientId(profile.tokenCredentials().tokenId() + UUID.randomUUID());
			mqttConfig.setReadTimeoutSeconds(0);
			mqttConfig.setWriteTimeoutSeconds(0);
	...
	private final class MqttConnectionService extends BaseMqttConnectionService
		...
		public void onMqttMessage(MqttMessage message) {
			System.out.println("Message on topic: " + message.getTopic());
			try {
				JsonNode msgBody = cborObjectMapper.readTree(message.getPayload());
				jsonOut.writeValue(out, msgBody);
				System.out.println();
			} catch (IOException e) {
